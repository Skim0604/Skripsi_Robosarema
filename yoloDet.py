...
class YoloTRT():
    ...
    def Inference(self, img):
        ...
        for j in range(len(result_boxes)):
            ...
            det["box"] = box 
            self.PlotBbox(box, img, color=self.colors[int(result_classid[j])], label="{}:{:.2f}".format(self.categories[int(result_classid[j])], result_scores[j]),)
    ...
    def PlotBbox(self, x, img, color, label=None, line_thickness=None):
        ...
        if label:
            ...
            cv2.rectangle(img, c1, c2, [0, 0, 0], -1, cv2.LINE_AA)  # filled
